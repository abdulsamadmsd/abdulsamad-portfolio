...
const ThemeToggle = ({ className = "" }: { className?: string }) => {
  ...
  return (
    <button
      ...
      className={`relative grid h-10 w-10 place-items-center overflow-hidden rounded-full border border-border bg-surface-elevated text-foreground transition-all hover:border-accent/60 hover:shadow-glow-accent group ${className}`}
    >
      {/* gradient halo */}
      <span
        aria-hidden
